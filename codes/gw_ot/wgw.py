...
class Entropic_WGW(nn.Module):
	...
	def __init__(self, eps=1e-4, lamda=1.0, max_iter=100, thresh=1e-9, w_max_iter=100, w_thresh=1e-9, w_cost_type='L2', inter_loss_type='square_loss', intra_cost_type='L2', data_precision='float', stable_sinkhorn=True, verbose=False):
	...
	def func_define(self):
		...
		if self.inter_loss_type == 'square_loss':
			...
			raise NotImplementedError('Inter loss type %s is not implemented!' %(self.inter_loss_type))
